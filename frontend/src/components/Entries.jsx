...
const PersonnelModal = ({ title, personnelList, onClose, isAbsent }) => {
  ...
  // Modal-local export helpers
  const exportModalCsv = useCallback(() => {
    const headers = isAbsent
      ? ["Name", "Department", "Role", "Reason"]
      : showCheckInTime
      ? ["Name", "Department", "Role", "Check-in"]
      : ["Name", "Department", "Role", "Status"];
    const rows = (computeViewList || []).map((p) => {
      ...
      if (isAbsent) {
        const lr =
          leaveIndex && leaveIndex.get ? leaveIndex.get(p.per_id) : null;
        const statusReason =
          p.per_status === "OnLeave"
            ? "On Leave"
            : p.per_status === "OnSickLeave"
            ? "Sick Leave"
            : null;
        if (lr?.request_type)
          col4 =
            lr.request_type === "Other"
              ? lr.request_other || "Other"
              : lr.request_type;
        else col4 = statusReason || "Absent";
      } else if (showCheckInTime) {
        col4 =
  ...
  }, [computeViewList, isAbsent, showCheckInTime, title, leaveIndex]);
  ...
  const exportModalPdf = useCallback(() => {
    const doc = new jsPDF();
    doc.setFontSize(14);
    doc.text(title, 14, 16);
    doc.setFontSize(10);
    let y = 24;
    const header = isAbsent
      ? "Name | Department | Role | Reason"
      : showCheckInTime
      ? "Name | Department | Role | Check-in"
      : "Name | Department | Role | Status";
    ...
    (computeViewList || []).forEach((p) => {
      ...
      doc.text(`${name} | ${dept} | ${role} | ${col4}`, 14, y);
      y += 6;
    });
    const dateStr = new Date().toISOString().slice(0, 10);
